...
router.get('/monitoring', requireAuth, async (req, res) => {
    try {
        ...
        const userEntries = await Promise.all(monitoredUsers.map(async (entry) => {
            let member = null;
            if (guild) {
...
router.post('/settings', requireAuth, async (req, res) => {
    try {
        ...
        if (payload.aiBatchPrompt !== undefined) normalized.aiBatchPrompt = payload.aiBatchPrompt; // New field
...
router.post('/verify/send-dm', requireAuth, async (req, res) => {
    ...
    try {
        ...
        const sent = await require('../utils/helpers').sendVerificationDM(member);
